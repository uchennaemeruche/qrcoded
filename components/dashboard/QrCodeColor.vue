                ...
                <div class="flow-root">
                    <ul role="list" class="divide-y divide-gray-200 dark:divide-gray-700">
                        <li class="py-3 sm:py-4">
                            BG: {{ background.color }}
                            <h6 class="text-xl font-normal leading-none text-gray-900 dark:text-white mb-2">{{ isGradient
                                    ? 'Gradient' : 'Single'
                            }} Background color</h6>
                            <div class="w-full" v-show="!transparentBackground">
                                <div class="w-full" v-if="!isGradient">
                                    <DashboardColorInput v-model="backgroundColor"
                                        @input="(value) => setBackgroundColor(value)" />
                                </div>
                                <div class="w-full" v-else>
                                    {{ background.gradient.type }}
                                    Gra:{{ background.gradient.colorStops[0].color }}
                                    <DashboardColorRadioButton v-model="background.gradient.type"
                                        :options="['linear', 'radial']" @input="(value) => $emit('update', {gradient:{type: value}})" />
                                    <div class="w-full flex mx-2 space-x-2">
                                        <DashboardColorInput v-model="background.gradient.colorStops[0].color"
                                            @input="(value) => $emit('update',{gradient:{colorStops:[{ offset: 0, color: value }]}})" />
                                        <DashboardColorInput v-model="background.gradient.colorStops[1].color"
                                            @input="(value) =>$emit('update',{gradient:{colorStops:[{ offset: 1, color: value }]}})" />
                                    </div>
                                </div>
                                <DashboardColorSwitcher title="Gradient Background" :item="isGradient"
                                    @input="(value) => switchToGradient(value)" />
                            </div>

                            <DashboardColorSwitcher title="Transparent Background" :item="transparentBackground"
                                @input="(value) => transparentBackground = value" />

                        </li>
                        <li class="py-3 sm:py-4">
                            <h6 class="text-xl font-normal leading-none text-gray-900 dark:text-white mb-2">Dots color
                            </h6>
                            <div class="flex items-center space-x-4 h-[35px] w-full" v-show="!isGradientDots">
                                ...
                                <div class="h-full w-1/5">
                                    <input type="color" class="h-full w-full" v-model="dotsColor"
                                        @change="setDotsColor(dotsColor)"
                                        style="border: 0.1rem solid rgb(227, 236, 242); background: rgb(255, 255, 255);">
                                </div>
                            </div>
                            <a
                                class="inline-flex space-x-4 justify-center items-center py-2 text-gray-500 rounded cursor-pointer ">
                        ...
                        <li class="py-3 sm:py-4">
                            <h6 class="text-xl font-normal leading-none text-gray-900 dark:text-white mb-2">Marker
                                border color</h6>
...
const { setBackgroundColor, background, updateColorStops, setDotsColor, backgroundColor, dotsColor, isGradientDots, markerBorderColor, markerCenterColor } = useQrCode()
...
const switchToGradient = (value) =>{
    isGradient.value = value
    emit('update', {gradient:{type: background.value.gradient.type}})
    emit('update', {gradient:{colorStops:[{ offset: 0, color: background.value.gradient.colorStops[0].color }, {offset: 1, color: background.value.gradient.colorStops[1].color}]}})
}
